...
def professores() -> str:
    ...
    professores: list[list[str]] = list(map(
        list, cursor.execute("""
                            SELECT id, nome, email, telefone, email
    ...
    for professor in professores:
        ...
        professor.insert(2, nome_materia)
...
def novo_professor() -> str:  
    ...
    if request.method == "POST":
        ...
        return render_template("novo_aluno.html",
                                Mensagem="Aluno cadastrado com sucesso!", Materias=materias)
...
Para criar o banco de dados, rode 'model.py'.
    ...
    port = int(os.environ.get("PORTA_SERVIDOR", 5000))
    app.run(host="0.0.0.0", debug=True, port=port)
>>>>>>> 83cef24 (feat: Implementa tela de formulário e tabela Prof)
